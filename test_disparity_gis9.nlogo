...
rich-own [utility-r diff-nearby nearby is_city home_quality home_price]
poor-own [utility-p diff-nearby nearby is_city home_price home_quality]
jobs-own [utility]
patches-own [quality price sd-dist obj_id kab flood-risk]
globals [counter view-mode min-poor-util max-poor-util min-rich-util max-rich-util dki percent-different poor-price-priority rich-quality-priority]
...
to setup
  clear-all
  set dki gis:load-dataset "/data_gis/Batas_Administrasi_DKI_Jakarta.shp"
  gis:set-world-envelope (gis:envelope-of dki)

  gis:set-drawing-color yellow
  gis:draw dki 1
  ;gis:apply-raster kec k_id
  foreach gis:feature-list-of dki [ feature ->
    ask patches gis:intersecting feature [
      set obj_id gis:property-value feature "OBJECTID"
      set kab gis:property-value feature "NO_KAB"
    ]
  ]
  ...
  setup-rich
  setup-poor
  ask patches with [obj_id != 0] [ update-patch-color ]
  set rich-quality-priority (traffic-level * (-1))
  set poor-price-priority (1 - traffic-level)
  reset-ticks
end
...
to setup-patches
  ask patches with [obj_id != 0 and kab = "74"] [
    set quality 70
    set price 23.0 + random-float 22
    set flood-risk 0.3 + random-float 0.2
  ]
  ask patches with [obj_id != 0 and kab = "75"] [
    set quality 49
    set price 12.0 + random-float 11
    set flood-risk 0.3 + random-float 0.6
  ]
  ask patches with [obj_id != 0 and kab = "71"] [
    set quality 49
    set price 12.0 + random-float 88
    set flood-risk 0.0 + random-float 0.3
  ]
  ask patches with [obj_id != 0 and kab = "73"] [
    set quality 49
    set price 6.5 + random-float 11
    set flood-risk 0.6 + random-float 0.3
  ]
  ask patches with [obj_id != 0 and kab = "72"] [
    set quality 27
    set price 1.0 + random-float 11
    set flood-risk 0.0 + random-float 0.3
  ]
  ask patches with [obj_id = 0 or price = 0] [
    set quality 25
    set price 1.0 + random-float 9
    ;set flood-risk 0.1 + random-float 0.5
    set flood-risk 0.0 + random-float 0.6
  ]
  ask patches
  [
    set sd-dist min [distance myself] of jobs
    set flood-risk (flood-risk * flood-level)
    ;set flag-poor 0
    ;set flag-rich 0
  ]
...
to setup-rich
  ...
  [
    ...
    ;ask patch-here [set flag-rich 1]
    raise-price
    raise-value
    set is_city 1
    set home_price [price] of patch-here
    set home_quality [quality] of patch-here
  ]
end
...
to setup-poor
  ...
  [
    ...
    setxy ( ( radius / 2 ) - random-float ( radius * 1.0 ) ) ( ( radius / 2 ) - random-float ( radius * 1.0 ) )
    ;decrease-price
    ;ask patch-here [set flag-poor 1]
    decrease-value
    set is_city 1
    set home_price [price] of patch-here
    set home_quality [quality] of patch-here
...
to evaluate-poor
  let candidate-patches n-of number-of-tests patches with [(pycor < 32) or (obj_id != 0)]
  ...
  ;ask best-candidate [set flag-poor 1]
  ifelse ([obj_id] of best-candidate != 0) [set is_city 1][set is_city 0]
  set home_price [price] of best-candidate
  set home_quality [quality] of best-candidate
end
...
to evaluate-rich
  let candidate-patches n-of number-of-tests patches with [((pycor < 32) or (obj_id != 0)) and (flood-risk < (0.3 + random-float 0.5))]
  set candidate-patches candidate-patches with [ not any? turtles-here ]
  if (not any? candidate-patches)
    [ stop ]
  ...
  ifelse ([obj_id] of best-candidate != 0) [set is_city 1][set is_city 0]
  ;ask best-candidate [set flag-rich 1]
  set home_price [price] of best-candidate
  set home_quality [quality] of best-candidate
end
...
to locate-service
  let empty-patches patches with [ not any? turtles-here and  obj_id != 0]
...
to evaluate-job
  ...
  set candidate-patches candidate-patches with [ not any? turtles-here and  obj_id != 0]
  if (not any? candidate-patches)
    [ stop ]
...
to update-view
  ...
  ifelse (view-mode = "flood")
  [
    ask patches [set pcolor black]
    ask patches with [obj_id != 0] [ update-patch-color ]
  ][
    ask patches with [(pycor < 32) or (obj_id != 0)] [ update-patch-color ]
  ]
...
to update-patch-color
  ...
  ifelse view-mode = "flood"
  [
   set pcolor scale-color brown flood-risk 1 0
  ][
...
to update-turtles
  ask turtles with [breed = rich or breed = poor] [
    set diff-nearby count (turtles-on neighbors) with [breed = [breed] of myself]
    set nearby count (turtles-on neighbors)
  ]
end

to update-globals
  let total-neighbors 0
  let diff-neighbors sum [diff-nearby] of turtles with [breed = rich or breed = poor]
  set total-neighbors sum [nearby] of turtles with [breed = rich or breed = poor]
  ;set percent-different (diff-neighbors / total-neighbors) * 100
  ifelse (total-neighbors != 0) [set percent-different (diff-neighbors / total-neighbors) * 100][set percent-different 0]
  ;set percent-different 10
end
@#$#@#$#@
GRAPHICS-WINDOW
350
...
<experiments>
  <experiment name="experiment-1" repetitions="10" runMetricsEveryStep="false">
    ...
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>percent-different</metric>
    <metric>count poor with [is_city = 1]</metric>
    <metric>count poor with [is_city = 0]</metric>
    <metric>count poor + count rich</metric>
    <metric>count poor</metric>
    <metric>count rich</metric>
    <metric>median [ min [distance myself] of jobs ] of rich</metric>
    <metric>median [ min [distance myself] of jobs ] of poor</metric>
    <metric>mean [home_price] of poor</metric>
    <metric>mean [home_price] of rich</metric>
    <metric>mean [home_quality] of poor</metric>
    <metric>mean [home_quality] of rich</metric>
    <enumeratedValueSet variable="death-rate-poor">
      <value value="9"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="number-of-tests">
      <value value="15"/>
    </enumeratedValueSet>
    <steppedValueSet variable="traffic-level" first="0" step="0.1" last="1"/>
    ...
    <enumeratedValueSet variable="death-rate-rich">
      <value value="2"/>
    </enumeratedValueSet>
  ...
  <experiment name="experiment-1" repetitions="10" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="flood-level" first="0" step="0.1" last="1"/>
  </experiment>
  <experiment name="exp_without_decrease_value" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>percent-different</metric>
  ...
  <experiment name="experiment-3" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>percent-different</metric>
    ...
    <enumeratedValueSet variable="traffic-level">
      <value value="0.8"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-jobs">
      <value value="10"/>
    ...
    <enumeratedValueSet variable="flood-level">
      <value value="0.8"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment-3" repetitions="10" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="number-of-tests" first="1" step="1" last="30"/>
    <enumeratedValueSet variable="traffic-level">
      <value value="0.8"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-jobs">
  ...
  <experiment name="experiment-traffic-flood-2" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>percent-different</metric>
  ...
  <experiment name="experiment-number-jobs" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>percent-different</metric>
    ...
    <steppedValueSet variable="max-jobs" first="5" step="1" last="20"/>
    <enumeratedValueSet variable="rich-per-step">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="death-rate-rich">
  ...
  <experiment name="experiment-traffic-flood-3" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>percent-different</metric>
    <metric>count poor with [is_city = 1]</metric>
    <metric>count poor with [is_city = 0]</metric>
    <metric>count rich with [is_city = 1]</metric>
    <metric>count rich with [is_city = 0]</metric>
    <metric>count poor + count rich</metric>
    <metric>count poor</metric>
    <metric>count rich</metric>
    <metric>median [ min [distance myself] of jobs ] of rich</metric>
